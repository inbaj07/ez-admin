...
exports.list = async (req, res) => {
  try {
    ...
    return res.status(500).send(comResponse(res, 500, false, constant.MSG_NO_DATA, doc.data))
  } catch (error) {
...
exports.listByID = async (req, res) => {
  try {
    ...
    return res.status(500).send(comResponse(res, 500, false, constant.MSG_NO_DATA, userDetails.data));
  } catch (error) {
    return res.status(500).send(comResponse(res, 500, false, constant.MSG_CATCH, error.toString()))
  }
}
exports.login = async (req, res) => {
  try {
    ...
    if (!userDetails.status) return res.status(500).send(comResponse(res, 500, false, constant.MSG_NO_DATA, constant.MSG_NO_DATA))
    ...
    if (passwordCheck) {
      ...
      if (!tokenUpdateDetails.status)
        return res.status(500).send(comResponse(res, 500, false, constant.MSG_TOKEN_FAILED, constant.MSG_TOKEN_FAILED))
    ...
    return res.status(500).send(comResponse(res, 500, false, constant.MSG_LOGIN_FAILED, constant.MSG_LOGIN_FAILED))
  }
  catch (error) {
    return res.status(500).send(comResponse(res, 500, false, constant.MSG_CATCH, error.toString()))
  }
...
exports.verifyToken = async (req, res) => {
  try {
    const data = jwt.verify(req.body.access_token, process.env.JWT_SECRET_KEY)
...
exports.NodeJSPracticeReadFile = async (req, res) => {
}
